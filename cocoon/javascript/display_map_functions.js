function initialize_map(id, path) {
	...
	/*************** OBJECT KML FEATURES ******************/
	objectControl = new OpenLayers.Control.SelectFeature(
                [kmlLayer],
                {
                    clickout: true, 
                    //toggle: true,
	...
	kmlLayer.events.on({"featureselected": onObjectSelect, "featureunselected": onFeatureUnselect});
	
	function onObjectSelect(event) {
		var feature = event.feature;
		message =  '<div style="font-size:10px">' + feature.attributes.name + '</div>';
